...
;       UPDATED to work on tropchem output rather than UCX 26/10/2017
;
...
pro shapefactor_from_trop_satellite_output, year, month
    ...
    isop   = dblarr(nlons,nlats, nlvls) ; molecules C/1e9 molec air
    E_isop = dblarr(nlons,nlats)        ; atoms c / cm2 / s
    ...
    foreach file, files, fi do begin
        ...
        flag= CTM_GET_DATABLOCK( isoptmp, 'IJ-AVG-$', Tracer=6, File=file ) 
        ; Read Emissions of isop:
        flag= CTM_GET_DATABLOCK( E_isoptmp, 'BIOGSRCE$', Tracer=1, File=file ) 
        ; Read psurf ( hPa )
    ...
    Nhcho   = double(ppbv)*double(Nair)*1d-9 ; molecs(hcho) / m3
    Nisop   = double(isop)*double(Nair)*1d-9 * 0.2; molecs(isop) /m3
    ...
    structdata={VCHCHO:Omega_H, NHCHO:Nhcho, ShapeZ:S_z, $
                ...
                E_ISOP:E_isop, ISOP:isop, NISOP:Nisop, $
    ...
    fout = './trop_shapefactor_'+yyyymm+'.he5'
    fid = H5F_CREATE(fout)
